...
/**
 * Provides tree-based file system navigation with sorted directory listings. Notifies listeners of
 * directory loading state changes and validates paths on initialization.
 */
public class DirectoryTreeModel implements TreeTableModel {
  private static final Logger LOGGER = LoggerFactory.getLogger(DirectoryTreeModel.class);
  ...
  /**
   * Loads directory contents synchronously and notifies listeners. Fires LOADING_STARTED before
   * I/O, DIRECTORIES_LOADED or LOADING_FAILED after.
   */
  public void loadDirectories() {
    ...
    try {
      ...
      LOGGER.error("Failed to load directories: {}", e.getMessage());
  ...
  private String validatePath(String path) {
    if (path == null || path.trim().isEmpty()) {
      LOGGER.info("Path is null or empty, using user.home");
      return System.getProperty("user.home");
    }

    File dir = new File(path);
    if (!dir.exists() || !dir.isDirectory()) {
      LOGGER.error("Invalid directory path: {}. Using user.home instead.", path);
      return System.getProperty("user.home");
    }
    return path;
  }
  ...
  public int getIndexOfChild(Object parent, Object child) {
    if (!(child instanceof File)) return -1;

    if (parent == root && rootFiles != null) {
      for (int i = 0; i < rootFiles.length; i++) {
        if (rootFiles[i].equals(child)) return i;
      }
    }
  ...
  public Object getValueAt(Object node, int column) {
    if (!(node instanceof File file)) return null;
  ...
  /**
   * Releases resources by clearing tree model listeners. PropertyChangeSupport listeners are
  ...
  public void dispose() {
    try {
      ...
      LOGGER.error("Error during DirectoryTreeModel disposal: {}", e.getMessage());
    }
  }
}
