...
public class StartFuzzersTask extends ProgressDialogWorker {
  ...
  public StartFuzzersTask(Component parent, List<FuzzerController> selectedPanels) {
  ...
  protected Void doInBackground() throws Exception {
    ...
    BurpExtender.MONTOYA_API
        .logging()
        .logToOutput("Starting " + selectedPanels.size() + " fuzzers...");
    ...
    for (int i = 0; i < selectedPanels.size(); i++) {
      ...
      updateProgress(
          i + 1, String.format("Started %d/%d: %s", i + 1, selectedPanels.size(), fuzzerName));
    }
  ...
  protected void done() {
    ...
    if (isCancelled()) {
      ...
      String message =
          String.format(
              "Start operation completed: %d successful, %d errors, %d skipped",
