...
import com.theblackturtle.mutafuzz.ui.fuzzer.ScriptComboBoxModel.ScriptEntry;
...
/**
 * Allows users to select and preview Python scripts from files or embedded resources. Displays
 * editable script content and persists the last selection.
 */
public class ScriptComboBoxPanel extends JXPanel {
   ...
   *
   * @param model Script model containing available scripts
   * @param api Montoya API instance for theme detection and preference persistence
   */
  public ScriptComboBoxPanel(ScriptComboBoxModel model, MontoyaApi api) {
    super();
    if (model == null) {
  ...
  /**
   * Force RSyntaxTextArea to recreate its global InputMaps/ActionMaps. Workaround for
   * RSyntaxTextArea Issue #269 where multiple ClassLoaders cause keymap conflicts in Burp Suite
   * plugin environment.
   */
  ...
  /**
   * Handle script selection from combo box. Persists selection to preferences and loads content for
   * preview.
   */
      ...
      if (selectedScript != null) {
        ...
        if (!selectedScript.isResource() && selectedScript.getFile() != null) {
          api.persistence()
              .preferences()
              .setString(
                  ScriptComboBoxModel.PREF_LATEST_SCRIPTS_PATH,
  ...
  /** Load script content for preview. Handles both file-based and resource-based scripts. */
    ...
    try {
      ...
      if (scriptEntry.isResource()) {
        ...
        if (content != null && !content.isEmpty()) {
          ...
          LOGGER.debug(
              "Resource script content loaded: {} ({} characters)",
              scriptEntry.getName(),
              content.length());
        } else {
        ...
        if (scriptFile != null && scriptFile.exists() && scriptFile.isFile()) {
          ...
          setScriptContent(content);
          LOGGER.debug(
              "File script content loaded: {} ({} characters)",
              scriptFile.getName(),
              content.length());
        } else {
          setScriptContent("-- Script file not found or not readable --");
          LOGGER.warn(
              "Script file not found or not readable: {}",
  ...
  private void setupDefaultSelection() {
    try {
      ...
      if (availableScripts != null && !availableScripts.isEmpty()) {
        ...
        String prefLatestScriptsPath =
            api.persistence().preferences().getString(ScriptComboBoxModel.PREF_LATEST_SCRIPTS_PATH);
  ...
  /**
   * Set selected script programmatically. Loads content since programmatic selection bypasses
   * action listeners.
   */
  ...
  /**
   * Get current text area content. Returns user-edited content which may differ from original file.
   */
  ...
  private void showError(String message) {
    ...
    if (message != null && !message.isEmpty()) {
      JOptionPane.showMessageDialog(
          this, message, "Script Selection Error", JOptionPane.ERROR_MESSAGE);
    }
  }
