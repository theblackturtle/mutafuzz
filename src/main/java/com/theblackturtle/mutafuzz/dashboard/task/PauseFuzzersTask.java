...
public class PauseFuzzersTask extends ProgressDialogWorker {
  ...
  protected Void doInBackground() throws Exception {
    ...
    BurpExtender.MONTOYA_API
        .logging()
        .logToOutput("Pausing " + selectedPanels.size() + " fuzzers...");
    ...
    for (int i = 0; i < selectedPanels.size(); i++) {
      ...
      updateProgress(
          i + 1, String.format("Paused %d/%d: %s", i + 1, selectedPanels.size(), fuzzerName));
    }
  ...
  protected void done() {
    ...
    if (isCancelled()) {
      ...
      String message =
          String.format(
              "Pause operation completed: %d successful, %d errors", successCount, errorCount);
