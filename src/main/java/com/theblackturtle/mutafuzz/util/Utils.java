...
/**
 * Provides string manipulation, random data generation, and formatting utilities. Supports
 * line/word counting, random identifiers, file size formatting, and line ending normalization.
 */
public class Utils {
  ...
  /**
   * Counts the number of lines in a string. Handles both LF (\n) and CR (\r) line endings.
   *
  ...
  /**
   * Counts the number of words in a string. Words are defined as sequences of letters or digits
  ...
  /**
   * Generates a random alphanumeric string. First character is always a letter to ensure valid
   * identifier format. Uses lowercase only to avoid case sensitivity issues.
   *
  ...
  /**
   * Normalizes all line endings to Unix-style LF. Handles both Windows (CRLF) and legacy Mac (CR)
   * line endings.
   *
   * @param str Input string
  ...
  /**
   * Formats byte size into human-readable string with appropriate unit. Units: B, KB, MB, GB, TB
   *
