...
public class WordlistTabbedPane extends JPanel {
  ...
  public WordlistTabbedPane(String defaultDirectoryPath, PreferenceManager preferenceManager) {
  ...
  private void loadPersistedTabs() {
    int count =
        preferenceManager.getIntPreference(DashboardConfigConstants.PREF_INPUT_PANEL_COUNT, 1);
  ...
  private void addTabInternal(int index, boolean selectNew) {
    ...
    InputPanel panel = new InputPanel(dirPath, baseKey, preferenceManager);
  ...
  private String getDirectoryPathForIndex(int index) {
    ...
    String path = preferenceManager.getPreference(key);
