...
public class StopFuzzersTask extends ProgressDialogWorker {
  ...
  public StopFuzzersTask(Component parent, List<FuzzerController> selectedPanels) {
  ...
  protected Void doInBackground() throws Exception {
    ...
    BurpExtender.MONTOYA_API
        .logging()
        .logToOutput("Stopping " + selectedPanels.size() + " fuzzers...");
    ...
    for (int i = 0; i < selectedPanels.size(); i++) {
      ...
      updateProgress(
          i + 1, String.format("Stopped %d/%d: %s", i + 1, selectedPanels.size(), fuzzerName));
    }
  ...
  protected void done() {
    ...
    if (isCancelled()) {
      ...
      String message =
          String.format(
              "Stop operation completed: %d successful, %d errors", successCount, errorCount);
