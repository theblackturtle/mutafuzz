...
/**
 * Executes Python fuzzing scripts in isolated Jython interpreter environments. Manages script
 * lifecycle including setup, execution, and cleanup with proper resource disposal.
 */
public class PythonScriptExecutor implements Runnable {
  ...
  /**
   * Creates executor with script bridge and wordlists. Separate PythonInterpreter created during
   * run() to isolate environments.
   *
   * @param scriptAttackHandler Bridge for Python-Java communication
   * @param scriptContent Python script to execute
   * @param wordlists List of wordlists (nullable)
   * @param rawHttpRequestResponses Raw HTTP request/response list for templates API (nullable)
   */
  ...
  private void setupInterpreter() {
    ...
    pythonInterpreter.set("wordlists", wordlists);
  ...
  private void executeScript() {
    ...
    if (envScript != null) {
      try {
        ...
        LOGGER.error(
            "PyException details: type={}, value={}, traceback available={}",
            e.type,
            e.value,
            e.traceback != null);
      }
  ...
  /**
   * Signals shutdown and blocks until cleanup completes. Invokes Python onStop callback if defined.
   */
  ...
  /**
   * Releases interpreter and nulls references to enable garbage collection. CAS ensures single
   * execution even if called from multiple threads.
   */
  ...
  private String getEnvironmentScript() {
    try (InputStream inputStream =
        PythonScriptExecutor.class.getClassLoader().getResourceAsStream("ScriptEnvironment.py")) {
      if (inputStream == null) {
