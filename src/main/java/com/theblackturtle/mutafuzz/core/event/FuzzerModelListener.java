package com.theblackturtle.mutafuzz.core.event;

import com.theblackturtle.mutafuzz.core.engine.FuzzerState;
import com.theblackturtle.mutafuzz.core.engine.RequestObject;

/**
 * Receives notifications from fuzzer engines about state changes, completed requests, and progress
 * metrics. Enables UI components and monitoring systems to track fuzzing activity in real-time.
 */
public interface FuzzerModelListener {

  /**
   * Notifies when fuzzer transitions between states (IDLE, RUNNING, PAUSED, STOPPED).
   *
  ...
  /**
   * Provides atomic snapshot of all fuzzer progress metrics. Atomic delivery prevents race
   * conditions between individual counter updates.
   *
