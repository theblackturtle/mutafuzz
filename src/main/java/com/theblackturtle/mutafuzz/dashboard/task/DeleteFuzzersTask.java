...
public class DeleteFuzzersTask extends ProgressDialogWorker {
  ...
  protected Void doInBackground() throws Exception {
    ...
    BurpExtender.MONTOYA_API
        .logging()
        .logToOutput("Deleting " + selectedPanels.size() + " fuzzers...");
    ...
    for (int i = 0; i < selectedPanels.size(); i++) {
      ...
      try {
        ...
        if (currentState != FuzzerState.RUNNING) {
          ...
          BurpExtender.MONTOYA_API
              .logging()
              .logToOutput("Cannot delete running fuzzer: " + fuzzerName);
        }
      } catch (Exception e) {
        errorCount++;
      ...
      updateProgress(
          i + 1, String.format("Deleted %d/%d: %s", i + 1, selectedPanels.size(), fuzzerName));
    }
  ...
  protected void done() {
    ...
    if (isCancelled()) {
      ...
      String message =
          String.format(
              "Delete operation completed: %d successful, %d errors", successCount, errorCount);
