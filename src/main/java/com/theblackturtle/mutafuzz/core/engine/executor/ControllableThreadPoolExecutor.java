...
/**
 * Thread pool executor that supports pausing and resuming task execution without canceling queued
 * tasks. Provides termination callbacks for cleanup operations.
 */
public class ControllableThreadPoolExecutor extends ThreadPoolExecutor
    implements ControllableExecutorService {

  private static final Logger LOGGER =
      LoggerFactory.getLogger(ControllableThreadPoolExecutor.class);
  ...
  /**
   * Blocks task execution if executor is paused. Threads wait at the pause gate until resume()
   * releases them.
   *
  ...
  /**
   * Pauses executor by blocking subsequent task execution. Active tasks continue running; queued
   * tasks wait until resume() is called.
   */
  @Override
  public void pause() {
  ...
  /**
   * Resumes executor by releasing all threads blocked at the pause gate. Queued tasks begin
  ...
  /**
   * Invokes all registered termination listeners after executor terminates. Exceptions in listeners
   * are caught and logged to prevent cascading failures.
   */
  @Override
  protected void terminated() {
    ...
    for (TerminationListener listener : terminationListeners) {
      try {
        ...
        LOGGER.error("Error in termination listener: {}", e.getMessage(), e);
      }
    }
  }
