...
/**
 * Displays HTTP request templates in three modes: Burp's request editor, blank panel for
 * script-driven fuzzing, or table of raw HTTP exchanges. Mode is fixed at creation time and
 * determines which UI component is displayed.
 */
public class RequestTemplatePanel extends JXPanel {
   ...
   * @param rawHttpListPanel Panel for RAW_HTTP_LIST mode (must be non-null for RAW_HTTP_LIST, null
   *     for other modes)
   * @param api Montoya API instance for creating request editors
   * @throws IllegalArgumentException if RAW_HTTP_LIST mode is specified without providing a panel
   */
  public RequestTemplatePanel(
      RequestTemplateMode mode, RawHttpListPanel rawHttpListPanel, MontoyaApi api) {
  ...
  /**
   * Sets the HTTP request to display in the editor. Only functional in REQUEST_EDITOR mode. No
  ...
  /**
   * Returns the current HTTP request from the editor, including any user modifications.
   *
  ...
  /**
   * Returns the raw HTTP request/response data. Only available in RAW_HTTP_LIST mode; returns empty
   * list for other modes.
   *
   * @return List of HttpRequestResponse objects, or empty list if not in RAW_HTTP_LIST mode
   */
  ...
  /**
   * Releases editor resources to prevent memory leaks. Should be called when the panel is no longer
   * needed.
   */
  public void dispose() {
