...
/**
 * Provides a resizable split-pane layout displaying the fuzzer session list alongside aggregated
 * results. Persists divider position and coordinates selection state.
 */
public class DashboardSplitPanePanel extends JPanel {
  ...
  private void saveDividerLocation(double ratio) {
    try {
      preferenceManager.setPreference(PREF_DASHBOARD_DIVIDER_RATIO, String.valueOf(ratio));
  ...
  private void restoreDividerLocation() {
    SwingUtilities.invokeLater(
        () -> {
          try {
            String savedRatio = preferenceManager.getPreference(PREF_DASHBOARD_DIVIDER_RATIO);
  ...
  /**
   * Releases all resources held by panel and its dependencies. Disposes in reverse dependency order
