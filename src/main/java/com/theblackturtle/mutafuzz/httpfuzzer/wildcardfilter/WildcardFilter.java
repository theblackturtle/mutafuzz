...
public class WildcardFilter {
  /**
   * User patterns - each "Ignore Requests" action creates one analyzer. Thread-safe for concurrent
   * read/write.
   */
  ...
  public boolean isWildcard(RequestObject requestObject) {
    BurpExtender.MONTOYA_API
        ...
        .logToOutput(
            "> matchesUserPattern: "
                + matchesUserPattern(requestObject)
                + ", matchesLearnPattern: "
                + matchesLearnPattern(requestObject)
                + ", both: "
                + ""
