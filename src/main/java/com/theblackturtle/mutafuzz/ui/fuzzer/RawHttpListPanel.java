...
/**
 * Displays HTTP request/response data in a sortable table with context menu operations. Supports
 * bulk deletion of selected entries and thread-safe data management.
 */
public class RawHttpListPanel extends JXPanel {
  ...
  /**
   * Table model for displaying HttpRequestResponse list with Id and URL columns. Read-only with
   * thread-safe CopyOnWriteArrayList backing.
   */
  private static class RawHttpListTableModel extends AbstractTableModel {
    ...
    /**
     * Removes multiple rows and fires table data changed event on EDT. Removes rows in descending
     * order to maintain index validity.
     *
