...
/**
 * Provides configuration controls for MutaFuzz settings including thread count, timeouts,
 * connection management, and requester engine selection. Automatically persists changes to
 * preferences.
 */
public class FuzzerOptionsPanel extends JXPanel {
   ...
   *
   * @param api Montoya API instance for preference persistence
   */
  public FuzzerOptionsPanel(MontoyaApi api) {
  ...
  private JPanel createSettingsPanel() {
    ...
    addLabeledComponentAt(
        contentPanel, gbc, 1, 0, "Force Close Connection:", forceCloseConnectionCheckBox);
    addLabeledComponentAt(contentPanel, gbc, 1, 2, "Follow Redirects:", followRedirectsCheckBox);
    addLabeledComponentAt(
        contentPanel, gbc, 2, 0, "Quarantine Threshold (0=disabled):", quarantineThresholdSpinner);
    addLabeledComponentAt(contentPanel, gbc, 2, 2, "Timeout (seconds):", timeoutSpinner);
    addLabeledComponentAt(
        contentPanel, gbc, 3, 0, "Max Requests per Connection:", maxRequestsPerConnectionSpinner);
    addLabeledComponentAt(
        contentPanel, gbc, 3, 2, "Max Connections per Host:", maxConnectionsPerHostSpinner);
  ...
  private void addLabeledComponentAt(
      JPanel panel,
  ...
  private void savePreferences() {
    ...
    try {
      ...
      preferences.setInteger(
          "fuzzer.retries.io.error", (Integer) retriesOnIOErrorSpinner.getValue());
      preferences.setInteger(
          "fuzzer.quarantine.threshold", (Integer) quarantineThresholdSpinner.getValue());
      preferences.setBoolean(
          "fuzzer.force.close.connection", forceCloseConnectionCheckBox.isSelected());
      ...
      preferences.setInteger(
          "fuzzer.max.requests.per.connection",
          (Integer) maxRequestsPerConnectionSpinner.getValue());
      preferences.setInteger(
          "fuzzer.max.connections.per.host", (Integer) maxConnectionsPerHostSpinner.getValue());
  ...
  /** Creates FuzzerOptions from current UI state. Called when starting fuzzer engine. */
