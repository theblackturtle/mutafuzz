...
/**
 * Resends selected HTTP requests in parallel with configurable thread count. Prompts for thread
 * count, executes requests asynchronously, and updates table rows with new responses. Displays
...
public class ResendRequestAction implements RequestTableAction<RequestObject> {
   ...
   */
  public ResendRequestAction(BurpHttpClient requester) {
  ...
  public void actionPerformed(RequestTableActionContext<RequestObject> context) {
    ...
    String input =
        JOptionPane.showInputDialog(
            context.getJTable(), "Number of threads:", String.valueOf(DEFAULT_THREADS));
    ...
    ResendWorker worker = new ResendWorker(context.getJTable(), requests, threadCount);
    worker.execute();
  }
  ...
  private class ResendWorker extends ProgressDialogWorker {
    ...
    protected Void doInBackground() throws Exception {
      LOGGER.debug(
          "Starting resend operation with {} threads for {} requests",
          threadCount,
          requests.size());
                    ...
                    try {
                      ...
                      SimpleHttpResponse response =
                          requester.sendRequest(
                              request.getHttpRequest().httpService(), request.getHttpRequest());
                      ...
                      updateProgress(
                          count, String.format("Resent %d/%d requests", count, requests.size()));
