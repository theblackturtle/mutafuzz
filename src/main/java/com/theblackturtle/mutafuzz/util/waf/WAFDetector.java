package com.theblackturtle.mutafuzz.util.waf;

import burp.api.montoya.http.message.responses.HttpResponse;

/**
 * Analyzes HTTP responses to detect WAF blocking and rate limiting. Identifies Cloudflare, Akamai,
 * Imperva, Sucuri, F5, Barracuda, AWS, Azure, and generic WAF fingerprints.
 */
public final class WAFDetector {
  ...
  private static boolean isCloudflareBlocked(HttpResponse httpResponse) {
    boolean hasCloudflareHeaders =
        httpResponse.headers().stream()
            .anyMatch(
                h -> {
    ...
    if (!hasCloudflareHeaders) return false;
    ...
    boolean hasCloudflareBodyPatterns =
  ...
  private static boolean isAkamaiBlocked(HttpResponse httpResponse) {
    ...
    if (!hasAkamaiHeaders) return false;

    int statusCode = httpResponse.statusCode();
    boolean isBlockStatusCode = (statusCode == 403);

    String body = getResponseBody(httpResponse).toLowerCase();
    boolean hasAkamaiBodyPatterns =
        body.contains("akamai")
                ...
                h -> {
                  ...
                  return name.contains("incap_ses")
                      ...
                      || (name.equals("set-cookie")
                          && (value.contains("incap_ses") || value.contains("visid_incap")))
                      || (name.equals("server")
                          && (value.contains("incapsula") || value.contains("imperva")));
                });

    String body = getResponseBody(httpResponse).toLowerCase();
    boolean hasImpervaBodyPatterns =
  ...
  private static boolean isSucuriBlocked(HttpResponse httpResponse) {
    ...
    String body = getResponseBody(httpResponse).toLowerCase();
    boolean hasSucuriBodyPatterns =
        body.contains("sucuri")
            ...
            .anyMatch(
                h -> {
                  ...
                  return name.contains("f5")
                      ...
                      || name.equals("x-hw")
                      || (name.equals("set-cookie")
                          && (value.contains("bigipserver") || value.contains("ts")))
                      || (name.equals("server") && value.contains("bigip"));
                });

    String body = getResponseBody(httpResponse).toLowerCase();
    boolean hasF5BodyPatterns =
        body.contains("f5")
  ...
  private static boolean isBarracudaBlocked(HttpResponse httpResponse) {
    ...
    String body = getResponseBody(httpResponse).toLowerCase();
    boolean hasBarracudaBodyPatterns =
        body.contains("barracuda")
            ...
            .anyMatch(
                h -> {
                  ...
                  return name.contains("aws")
                      ...
                      || (name.equals("server")
                          && (value.contains("awselb")
                              || value.contains("amazon")
                              || value.contains("aws")));
                });

    String body = getResponseBody(httpResponse).toLowerCase();
    boolean hasAwsWafBodyPatterns =
        body.contains("aws")
  ...
  private static boolean isAzureWafBlocked(HttpResponse httpResponse) {
    ...
    String body = getResponseBody(httpResponse).toLowerCase();
    boolean hasAzureWafBodyPatterns =
        body.contains("azure")
  ...
  private static boolean isGenericWafBlocked(HttpResponse httpResponse) {
    ...
    String body = getResponseBody(httpResponse).toLowerCase();
    boolean hasGenericWafBodyPatterns =
        body.contains("waf")
    ...
    return isBlockStatusCode && hasGenericWafBodyPatterns;
  }
