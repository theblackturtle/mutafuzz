...
/**
 * Sends HTTP requests using Burp Suite's native HTTP client with configurable redirect handling and
 * timeout support.
 */
public class BurpRequester implements HTTPRequesterInterface {
  ...
  private void initRequestOptions() {
    ...
    this.requestOptions = RequestOptions.requestOptions().withRedirectionMode(redirectionMode);
  }
  ...
  @Override
  public MyHttpRequestResponse sendRequest(
      HttpService httpService, HttpRequest httpRequest, int redirectCount)
      throws IOException, RuntimeException, InterruptedException {
    if (!isRunning) {
    ...
    MyHttpRequestResponse myHttpRequestResponse =
        new MyHttpRequestResponse(httpService, httpRequest);
