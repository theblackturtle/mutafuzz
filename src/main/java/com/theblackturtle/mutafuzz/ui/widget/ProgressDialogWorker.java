package com.theblackturtle.mutafuzz.ui.widget;
...
/**
 * Base SwingWorker class with integrated ProgressMonitor for long-running operations. Provides
 * EDT-safe progress updates and automatic cleanup.
 *
 * <p>Usage example:
 *
 * <pre>
...
public abstract class ProgressDialogWorker extends SwingWorker<Void, String> {
  ...
  protected ProgressDialogWorker(Component parent, String title, int totalItems) {
    ...
    this.progressMonitor = new ProgressMonitor(parent, title, "Initializing...", 0, totalItems);
    this.progressMonitor.setProgress(0);
  }

  /**
   * Updates progress display with current status. Thread-safe and can be called from the background
   * thread.
   *
  ...
  /**
   * Called on EDT when publish() is invoked from background thread. Updates the ProgressMonitor
  ...
  /**
   * Called on EDT when background task completes. Automatically closes the progress monitor.
