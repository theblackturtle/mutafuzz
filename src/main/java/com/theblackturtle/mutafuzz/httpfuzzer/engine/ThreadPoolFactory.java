...
/**
 * Constructs thread pools configured for fuzzing workloads with bounded queues, automatic
 * backpressure, and termination callbacks.
 */
public class ThreadPoolFactory {
  ...
  public static ControllableExecutorService createFuzzerTaskExecutor(
      FuzzerOptions fuzzerOptions, HttpFuzzerEngine fuzzerEngine, String namePrefix) {
    ...
    try {
      ...
      BlockingQueue<Runnable> workQueue =
          new ArrayBlockingQueue<>(fuzzerOptions.getThreadCount() * 2, true);
      RejectedExecutionHandler rejectionHandler = createRejectionHandler();

      ControllableThreadPoolExecutor executor =
      ...
      LOGGER.debug(
          "Created thread pool executor: {} threads, queue capacity: {}",
          fuzzerOptions.getThreadCount(),
          fuzzerOptions.getThreadCount() * 2);
  ...
  /** Creates thread factory ensuring non-daemon threads to prevent premature JVM shutdown. */
  ...
  /**
   * Implements backpressure by blocking producers when queue saturates. Mimics Go channel behavior:
   * blocks on send when buffer is full. Caller thread waits until space is available (never
   * discards tasks).
   */
