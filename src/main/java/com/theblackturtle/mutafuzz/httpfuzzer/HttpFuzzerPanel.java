...
public class HttpFuzzerPanel extends JFrame implements FuzzerModelListener {
   ...
   */
  public HttpFuzzerPanel(
      int fuzzerId, String identifier, HttpRequest templateRequest, FuzzerOptions fuzzerOptions) {
          ...
          try {
            ...
            try {
              ...
              LOGGER.debug(
                  ...
                  currentScriptContent != null && !currentScriptContent.isEmpty()
                      ? "LOADED"
                      : "EMPTY",
                  wordlists.size(),
                  totalPayloads,
            ...
            if (fuzzerEngine == null) {
              LOGGER.error(
                  "CRITICAL: Failed to recreate HttpFuzzerEngine after UI sync for fuzzer: {}",
                  identifier);
              SwingUtilities.invokeLater(
                  () -> {
  ...
  public void addFuzzerModelListener(FuzzerModelListener listener) {
    if (listener != null && !modelListeners.contains(listener)) {
      ...
      LOGGER.debug("Added listener to panel {} (total: {})", fuzzerId, modelListeners.size());
    }
  }
  ...
  public void removeFuzzerModelListener(FuzzerModelListener listener) {
    if (listener != null) {
      ...
      LOGGER.debug("Removed listener from panel {} (total: {})", fuzzerId, modelListeners.size());
    }
  }
        ...
        new Thread(
            () -> {
              try {
                ...
                if (!worker.isDone()) {
                  LOGGER.warn(
                      "Disposal exceeded 5s timeout, forcing close for fuzzer: {}", identifier);
                  SwingUtilities.invokeLater(
                      () -> {
                        progressDialog.setVisible(false);
                        progressDialog.dispose();
