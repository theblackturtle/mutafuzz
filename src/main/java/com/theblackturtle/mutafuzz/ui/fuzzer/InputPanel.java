...
/**
 * Manages payload input through three sources: manual entry, directory browser, and most-used
 * files. Tracks file usage statistics and persists tab selections between sessions.
 */
public class InputPanel extends JTabbedPane {
  ...
  public InputPanel(
      String defaultDirectoryPath, String basePreferenceKey, PreferenceManager preferenceManager) {
    super();
  ...
  private void initializeTabs() {
    ...
    // Most Used tab
    mostUsedPanel = new MostUsedPanel(basePreferenceKey + SUFFIX_MOST_USED, preferenceManager);
    addTab(MOST_USED_TAB_NAME, mostUsedPanel);

    // Directory tab
    directoryTreePanel =
        new DirectoryTreePanel(
            defaultDirectoryPath, basePreferenceKey + SUFFIX_DIRECTORY_TREE, preferenceManager);
    ...
    addChangeListener(
        e -> {
          ...
          if (selectedIndex >= 0) {
            preferenceManager.setIntPreference(preferenceKeyLastTab, selectedIndex);
    ...
    int lastSelectedTab =
        preferenceManager.getIntPreference(preferenceKeyLastTab, PAYLOADS_TAB_INDEX);
  ...
  /**
   * Returns aggregated payloads from the currently selected tab. Automatically tracks file usage
   * statistics for Directory and Most Used tabs.
   *
  ...
  public List<String> getPayloads() {
    ...
    switch (selectedTab) {
      ...
      case MOST_USED_TAB_INDEX:
        List<File> mostUsedFiles = mostUsedPanel != null ? mostUsedPanel.getSelectedFiles() : null;
        trackFileUsage(mostUsedFiles);
        return readAndDeduplicateFiles(mostUsedFiles);

      case DIRECTORY_TAB_INDEX:
        List<File> directoryFiles =
            directoryTreePanel != null ? directoryTreePanel.getSelectedFiles() : null;
