package com.theblackturtle.mutafuzz.core.filter;
...
public class VariationsAnalyzer implements ResponseVariationsAnalyzer {
   ...
   *
   * <p>Thread-safe: synchronized to prevent concurrent modification of internal state when multiple
   ...
   * the baseline values.
   *
   * <p>Thread-safe: synchronized to ensure consistent reads of internal state while other threads
   * may be calling updateWith().
   *
   * @param httpResponse response to check for similarity
   * @return true if all invariant attributes match the baseline values, false if no patterns have
   *     been learned yet
   */
  public synchronized boolean isSimilar(HttpResponse httpResponse) {
    // No patterns learned yet - can't match anything
