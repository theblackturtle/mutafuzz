...
/**
 * Adds MutaFuzz menu items to Burp Suite's context menus. Enables sending single requests with text
 * selection or multiple raw HTTP requests to the fuzzer.
 */
public class FuzzerContextMenuItemsProvider implements ContextMenuItemsProvider {
  ...
  public List<Component> provideMenuItems(ContextMenuEvent event) {
    ...
    if (event.invocationType() == InvocationType.MESSAGE_EDITOR_REQUEST
      ...
      JMenuItem openHTTPFuzzerSelectingText = new JMenuItem("Open MutaFuzz (selecting text)");
  ...
  /**
   * Handles menu action for opening fuzzer with text selection. Creates fuzzer in REQUEST_EDITOR
   * mode, replacing selected text with %s placeholder.
   */
  private void handleOpenWithSelection(ContextMenuEvent event) {
    Optional<MessageEditorHttpRequestResponse> httpRequestResponse =
    ...
    if (httpRequestResponse.isPresent()) {
      ...
      if (selectionOffsets.isPresent()) {
        ...
        requestStr =
            requestStr.substring(0, selectionOffset.startIndexInclusive())
                + "%s"
  ...
  /**
   * Handles menu action for sending multiple requests to fuzzer. Creates fuzzer in RAW_HTTP_LIST
   * mode with all selected HTTP request/response pairs.
   */
  private void handleSendMultipleRequests(ContextMenuEvent event) {
    ...
    dashboard.createFuzzerFromBurp(null, true, options);
  }
}
