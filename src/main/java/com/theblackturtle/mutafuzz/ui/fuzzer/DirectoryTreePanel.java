...
/**
 * Displays directory hierarchies in an expandable tree table format. Supports file selection, path
 * copying, and persists selection state across sessions.
 */
public class DirectoryTreePanel extends JPanel {
   ...
   * @param preferenceManager Manager for persisting preferences
   */
  public DirectoryTreePanel(
      String path, String preferenceKey, PreferenceManager preferenceManager) {
  ...
  private void copySelectedPath() {
    ...
    if (selectedFile != null) {
      ...
      JOptionPane.showMessageDialog(
          this, "Path copied to clipboard", "Information", JOptionPane.INFORMATION_MESSAGE);
    }
  }

  private void handleSelectionChange() {
    if (preferenceKey != null && preferenceManager != null) {
      File selectedFile = getSelectedFile();
      if (selectedFile != null) {
        String componentPath = buildComponentPath(selectedFile);
        if (componentPath != null) {
          preferenceManager.setPreference(preferenceKey, componentPath);
        }
      }
    }
  }
  ...
  private void restoreSelection() {
    ...
    String componentPath = preferenceManager.getPreference(preferenceKey);
  ...
  private void handleModelChange(PropertyChangeEvent evt) {
    ...
    switch (propertyName) {
      ...
      case DirectoryTreeModel.LOADING_FAILED:
        ...
        JOptionPane.showMessageDialog(
            this, "Failed to load directories: " + error, "Error", JOptionPane.ERROR_MESSAGE);
        break;
    }
  }
  ...
  public List<File> getSelectedFiles() {
    if (treeTable == null) return new ArrayList<>();

    int[] selectedRows = treeTable.getSelectedRows();
    if (selectedRows.length == 0) return new ArrayList<>();
