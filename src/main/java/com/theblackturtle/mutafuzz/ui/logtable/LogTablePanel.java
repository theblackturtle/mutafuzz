package com.theblackturtle.mutafuzz.ui.logtable;
...
/**
 * Displays fuzzer results in a sortable table with request/response viewer and context menu
 * actions. Supports adding, filtering, and removing requests with wildcard pattern matching.
 */
public class LogTablePanel extends JPanel implements FuzzerModelListener {
  ...
  public LogTablePanel(
    ...
    buildUI(preferenceManager);
    setupActions();

    LOGGER.debug("Created LogTablePanel for fuzzer: {} ({})", fuzzerId, identifier);
  }

  private void buildUI(PreferenceManager preferenceManager) {
  ...
  private void notifySelectionListeners() {
    for (ListSelectionListener listener : new ArrayList<>(externalListeners)) {
      listener.valueChanged(
          new ListSelectionEvent(requestTable.getTable().getSelectionModel(), -1, -1, false));
    }
  }
  ...
  public List<RequestObject> getAllRequests() {
    ...
    try {
      ...
      LOGGER.error(
          "Error getting all requests from log table {}: {}", identifier, e.getMessage(), e);
      return List.of();
    }
  }
  ...
  public List<RequestObject> getFilteredRequests() {
    ...
    try {
      ...
      LOGGER.error(
          "Error getting filtered requests from log table {}: {}", identifier, e.getMessage(), e);
      return List.of();
    }
  }
  ...
  public int getRequestCount() {
    ...
    try {
      ...
      LOGGER.error(
          "Error getting request count from log table {}: {}", identifier, e.getMessage(), e);
      return 0;
    }
  }
  ...
  public void onResultAdded(int fuzzerId, RequestObject result, boolean interesting) {
    if (isDisposed.get() || result == null) return;
    SwingUtilities.invokeLater(() -> addRequest(result));
  }

  @Override
  public void onStateChanged(int fuzzerId, FuzzerState newState) {
    // No-op - state changes handled by dashboard
  }

  @Override
  public void onCountersUpdated(
      int fuzzerId, long completedCount, long totalCount, long errorCount) {
    // No-op - counters handled by dashboard and frame
  }

  @Override
  public void onFuzzerDisposed(int fuzzerId) {
    // No-op - disposal handled by FuzzerSession
  }
