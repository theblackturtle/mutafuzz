...
public class FuzzerTask implements Runnable {
  ...
  private boolean executeRequest() throws Exception {
    ...
    try {
      ...
      SimpleHttpResponse response = executeWithRetries(message, maxRetries);
  ...
  private SimpleHttpResponse executeWithRetries(HttpRequest request, int maxRetries)
      throws Exception {
    HttpClientBase client = engine.getHttpClient();
    if (client == null) {
    ...
    for (int attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        SimpleHttpResponse response = client.sendRequest(request.httpService(), request);
        if (attempt > 0) {
  ...
  private RequestObject createResult(SimpleHttpResponse response, long responseTime) {
