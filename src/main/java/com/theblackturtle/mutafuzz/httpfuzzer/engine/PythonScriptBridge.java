...
/**
 * Provides Python scripts with access to fuzzing operations: queuing requests, registering result
 * callbacks, sending synchronous requests, and managing execution lifecycle. Includes utility
 * methods for encoding, hashing, and session state management.
 */
public class PythonScriptBridge {
  ...
  /**
   * @param httpFuzzerEngine Pre-initialized engine instance that this bridge will control
   */
  ...
  /**
   * Registers a Python callback to process fuzzing results, automatically converting types
   *
   * @param pyFunction Python function that receives (RequestObject, Boolean) for each result
   */
  ...
  /**
   * Primary queue method accepting pre-built HttpRequest. Most flexible option - allows full
   * control via Montoya API.
   *
  ...
  /**
   * Queues payloads for fuzzing using original request template. Original request must contain %s
  ...
  /**
   * Send HTTP request synchronously and return response immediately. Delegates to HttpFuzzerEngine
   * for execution.
   *
  ...
  /**
   * Send URL request synchronously and return response immediately. Delegates to HttpFuzzerEngine
   * for execution.
   *
  ...
  /**
   * Send request with payloads synchronously and return response immediately. Delegates to
   * HttpFuzzerEngine for execution.
   *
  ...
  /**
   * Send request with raw template and payloads synchronously. Delegates to HttpFuzzerEngine for
   * execution.
   *
   * @param url Target URL for the request
   * @param requestTemplate Raw HTTP request template with %s markers
  ...
  /**
   * Retrieves current HTTP request from template editor. Allows Python scripts to queue user-edited
   * requests.
   *
  ...
  /**
   * Blocks until the fuzzer engine is ready to accept tasks or times out. Python scripts must call
   * this after configuration but before queuing tasks to ensure the executor is initialized.
   *
   * @return true if engine initialized successfully, false if timeout or error occurred
   */
  public boolean startEngine() {
    ...
    while (waitedTime < MAX_WAIT_TIME) {
      try {
        if (httpFuzzerEngine.isFuzzerTaskExecutorInitialized()) {
          LOGGER.debug(
              "fuzzerTaskExecutor has been initialized successfully after {} ms", waitedTime);
          return true;
        }
    ...
    if (httpFuzzerEngine.isFuzzerTaskExecutorInitialized()) {
      LOGGER.debug(
          "fuzzerTaskExecutor has been initialized successfully (with delay) after {} ms",
          MAX_WAIT_TIME);
      return true;
    }

    LOGGER.warn(
        "Timeout while waiting for fuzzerTaskExecutor to be initialized ({}ms)", MAX_WAIT_TIME);
    return false;
  }
  ...
  /**
   * Bypasses callback filtering to forcibly add a result to the UI table. Use when script logic
   * determines a request is interesting regardless of callback decision.
   *
  ...
  /**
   * Signals that no more tasks will be queued, allowing engine to complete when queue drains.
   * Scripts must call this at the end of queueTasks() to prevent indefinite waiting.
   */
  public void done() {
    if (httpFuzzerEngine != null) {
  ...
  /**
   * Releases engine reference when script execution completes to prevent memory leaks. The final
   * reference cannot be nulled, but logging aids debugging of lifecycle issues.
   */
  public void cleanup() {
    if (httpFuzzerEngine != null) {
      LOGGER.debug(
          "Cleaning up PythonScriptBridge for engine: {}", httpFuzzerEngine.getDisplayName());
    }
    sessionData.clear();
  }
  ...
  /**
   * Escape a string for JSON (RFC 8259 compliant). Escapes double quotes, backslashes, and control
   * characters. Does NOT add surrounding quotes - caller handles that.
   *
  ...
  /**
   * Unescape a JSON-escaped string (RFC 8259 compliant). Reverses escape sequences created by
   * jsonEscape(). Handles: double-quote, backslash, forward-slash, backspace, form-feed, newline,
   * carriage-return, tab, and Unicode escape sequences.
   *
  ...
  /**
   * Store a value in session state for multi-step fuzzing workflows. Thread-safe storage shared
   * across all script callbacks.
   *
